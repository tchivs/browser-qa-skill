...
浏览器 QA 现在采用**失败即关闭（fail-closed）的就绪门禁**。打开浏览器前，agent 必须确认项目根目录、部署模式、准确的启动/健康检查命令、已验证的前端 URL、认证要求、凭据来源和 QA 范围。部署或认证信息缺失时必须继续发现或明确标记阻塞，不能猜测。
...
如需本地持久化凭据，profile 仍只保存来源引用。只有用户明确授权后，明文凭据才可写入被 Git 忽略的 `.browser-qa/env.local`，而 `.browser-qa/env.example` 只保存变量名和占位符。生产环境默认只读；不同环境使用隔离的浏览器会话；每次运行记录证据、重试结果和清理情况。
